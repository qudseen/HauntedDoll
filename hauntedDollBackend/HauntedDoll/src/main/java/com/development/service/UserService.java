...
public class UserService{
   ...
   @Path("/getOrders")
   @Produces(MediaType.APPLICATION_XML)
   public List<Order> getOrders(@QueryParam("userid") int userId){
      try {
   ...
   @Path("/placeOrder")
   @Produces(MediaType.APPLICATION_XML)
   public Status placeOrder(@QueryParam("userid") int userid, @QueryParam("address") String address){
	  Status sts = new Status("");
      try {
		orderDAO.placeOrder(cartDAO, userid, address);
		sts.setStatus("Success");
		return sts;
	} catch (SQLException e) {
		// TODO Auto-generated catch block
